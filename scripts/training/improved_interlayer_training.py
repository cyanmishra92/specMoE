#!/usr/bin/env python3
"""
Improved Inter-Layer Speculation Training
Enhanced version with 150 epochs and better convergence
"""
...
class EnhancedSpeculativeDataset(torch.utils.data.Dataset):
    """Enhanced dataset for inter-layer speculation"""
    ...
    def __init__(self, traces, max_layers=12, context_length=3, prediction_horizon=2):
        ...
        logger.info(f"Processing traces for inter-layer speculation...")
...
def collate_fn(batch):
    ...
    for item in batch:
        ...
        if seq_len < max_seq_len:
            ...
            context = F.pad(context, (0, 0, 0, pad_size), value=0)
            target = F.pad(target, (0, pad_size), value=0)
...
def train_enhanced_interlayer():
    """Train enhanced inter-layer speculation model"""
    
    # Enhanced configuration for 150 epochs
    config = {
        'num_experts': 128,
        'hidden_size': 512,
        'num_layers': 12,
        'model_dim': 384,           # Increased from 320
        'num_heads': 12,            # Increased from 10
        'ff_dim': 1536,             # Increased from 1280
        'num_attention_layers': 6,  # Increased from 5
        'dropout': 0.1,             # Reduced from 0.12
        'context_length': 3,
        'prediction_horizon': 2,
        'batch_size': 32,           # Increased from 28
        'learning_rate': 8e-5,      # Increased from 6e-5
        'num_epochs': 150,          # Extended training
        'warmup_steps': 1000,       # Increased warmup
        'weight_decay': 0.01,       # Reduced from 0.012
        'gradient_clip': 1.0,       # Increased from 0.8
        'label_smoothing': 0.05,    # Reduced from 0.06
        'patience': 15,             # Early stopping patience
        'min_lr': 1e-6              # Minimum learning rate
    }
    
    logger.info("Starting Enhanced Inter-Layer Speculation Training")
    ...
    # Create dataset
    dataset = EnhancedSpeculativeDataset(
        traces,
    ...
    for epoch in range(1, config['num_epochs'] + 1):
        ...
        for batch_idx, batch in enumerate(train_pbar):
            ...
            # Forward pass
            expert_logits, confidence, pattern_weights = model(context_experts, context_layers)
            
            # Reshape for loss calculation
            batch_size, seq_len, num_experts = expert_logits.shape
            expert_logits = expert_logits.view(-1, num_experts)
            target_experts = target_experts.view(-1)
            
            # Calculate loss
            loss = criterion(expert_logits, target_experts)
            ...
            train_targets.extend(target_experts.cpu().numpy())
        ...
        with torch.no_grad():
            ...
            for batch in val_pbar:
                ...
                loss = criterion(expert_logits, target_experts_flat)
        ...
        val_confidences_tensor = torch.cat(val_confidences, dim=0) if val_confidences else None
        ...
        if val_confidences_tensor is not None:
        ...
        if val_accuracy > best_val_accuracy:
            ...
            best_model_path = f"models/enhanced_interlayer_best_{datetime.now().strftime('%Y%m%d_%H%M%S')}.pth"
            torch.save({
                'epoch': epoch,
    ...
    history_path = f"models/enhanced_interlayer_history_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
