#!/usr/bin/env python3
"""
Collect Qwen1.5-MoE-A2.7B Traces for Expert Speculation Training
Optimized for RTX 3090 and A6000 GPUs - Much smaller than Mixtral!
"""
...
class QwenMoETraceCollector:
    """Collector for Qwen1.5-MoE-A2.7B traces - RTX 3090/A6000 optimized"""
    ...
    def _select_best_gpu(self):
        ...
        try:
            ...
            # Select best GPU - Qwen1.5-MoE is much smaller than Mixtral
            selected_gpu = None
            for gpu in sorted_gpus:
                if gpu.memoryFree >= 8:  # Only 8GB needed for Qwen1.5-MoE
                    selected_gpu = gpu
                    break
            
            if selected_gpu is None:
                selected_gpu = sorted_gpus[0]
                logger.warning(f"No GPU with 8GB+ free memory found, using GPU {selected_gpu.id} with {selected_gpu.memoryFree:.1f}GB free")
            ...
            return "cuda", {"id": 0, "name": "CUDA", "memory_total": 24, "memory_free": 24}  # Assume RTX 3090
    ...
    def _get_optimal_config_for_gpu(self):
        ...
        if memory_gb >= 40:  # A6000 48GB or A100
            return {
                "quantization": "8bit",  # Qwen1.5-MoE is small enough for 8bit
                "device_map": "auto",
            ...
            return {
                "quantization": "8bit",
                "device_map": "auto",
                "max_memory": {gpu_id: f"{int(memory_gb * 0.85)}GB"},
                "cpu_offload": False
            }
        else:  # Smaller GPUs
            return {
                "quantization": "8bit",
                "device_map": "auto",
                "max_memory": {gpu_id: f"{int(memory_gb * 0.8)}GB", "cpu": "10GB"},
                "cpu_offload": True
            }
        
    def load_model(self):
        """Load Qwen1.5-MoE-A2.7B model"""
        try:
            logger.info("Loading Qwen1.5-MoE-A2.7B model...")
            ...
            if not model_loaded:
                raise RuntimeError("Failed to load any Qwen1.5-MoE model. Please check model access or GPU memory.")
            
        except Exception as e:
            logger.error(f"❌ Error loading Qwen1.5-MoE model: {e}")
            raise

    def extract_expert_routing(self, outputs, inputs):
        """Extract expert routing information from Qwen1.5-MoE outputs"""
        routing_data = []
    ...
    def collect_traces_from_text_batch(self, texts: List[str], dataset_names: List[str], sample_ids: List[str], batch_size: int = 16):
    ...
    def collect_from_datasets(self, target_total_traces=20000, max_traces_per_sample=150):
        ...
        # Dataset selection optimized for Qwen1.5-MoE - clean datasets
        dataset_configs = [
            ("imdb", None, "train"),
        ...
        for dataset_name, config_name, split in dataset_configs:
            ...
            try:
                ...
                initial_samples = min(30, dataset_size)
                ...
                while len(dataset_traces) < traces_per_dataset and sample_idx < len(indices):
                    ...
                    if sample_idx >= len(indices) and len(dataset_traces) < traces_per_dataset:
                        additional_samples = min(30, dataset_size - len(indices))
    ...
    def save_traces(self, traces, output_dir="routing_data"):
        ...
        # Save as pickle
        pickle_path = output_dir / "qwen15_moe_a27b_traces.pkl"
        ...
        json_path = output_dir / "qwen15_moe_a27b_traces.json"
...
def main():
    ...
    logger.info("🚀 Starting Qwen1.5-MoE-A2.7B Trace Collection")
    ...
    # Collect traces - reduced target for faster collection
